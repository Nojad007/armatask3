...
resource "google_compute_region_health_check" "task3backendhc" {
  name                = "task3backendhc"
  # region = "us-central1"
...
resource "google_compute_region_backend_service" "task3backendserv" {
  name                  = "task3backendserve"
  ...
  health_checks         = [google_compute_region_health_check.task3hc.self_link]
